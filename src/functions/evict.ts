...
import type { Session, CompressedObservation, SessionSummary } from "../types.js";
...
export function registerEvictFunction(sdk: ISdk, kv: StateKV): void {
  sdk.registerFunction(
    { id: "mem::evict", description: "Evict stale memories based on age and importance" },
    async (data: { dryRun?: boolean }): Promise<EvictionStats> => {
      const ctx = getContext();
      const dryRun = data?.dryRun ?? false;

      const configOverride = await kv.get<Partial<EvictionConfig>>(KV.config, "eviction").catch(() => null);
      const cfg = { ...DEFAULTS, ...configOverride };

      const now = Date.now();
      const stats: EvictionStats = { staleSessions: 0, lowImportanceObs: 0, capEvictions: 0, dryRun };
      ...
      for (const session of sessions) {
        ...
        const staleDays = cfg.staleSessionDays * 24 * 60 * 60 * 1000;
      ...
      for (const session of sessions) {
        const obs = await kv.list<CompressedObservation>(KV.observations(session.id)).catch(() => []);
        const compressed = obs.filter((o) => o.title);

        for (const o of compressed) {
          if (!o.timestamp) continue;
          const age = now - new Date(o.timestamp).getTime();
          const maxAge = cfg.lowImportanceMaxDays * 24 * 60 * 60 * 1000;
          if (age > maxAge && (o.importance ?? 5) < cfg.lowImportanceThreshold) {
      ...
      for (const [, obs] of projectObs) {
        if (obs.length > cfg.maxObservationsPerProject) {
          const sorted = obs.sort((a, b) => (a.importance ?? 5) - (b.importance ?? 5));
          const toEvict = sorted.slice(0, obs.length - cfg.maxObservationsPerProject);
