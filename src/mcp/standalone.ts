...
function validate(toolName: string, args: Record<string, unknown>): Validated {
  ...
  switch (toolName) {
    case "memory_save": {
      ...
      const saveProject = args["project"];
      if (typeof saveProject === "string" && saveProject.trim()) {
        v.project = saveProject.trim();
      }
      return v;
    }
    case "memory_recall":
...
async function handleProxyGeneric(
  ...
  // reach lessons, sentinels, slots, signals, graph, and other tools
