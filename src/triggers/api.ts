...
import { CONSOLIDATION_LAST_RUN_KEY, PROCEDURAL_MIN_SESSIONS_PER_PATTERN, describeConsolidation, type ConsolidationRunRecord } from "../functions/consolidation-status.js";
...
export function registerApiTriggers(
  ...
  sdk.registerFunction("api::consolidation-status",
    async (req: HttpRequest): Promise<Response> => {
      const authErr = checkAuth(req, secret);
      if (authErr) return authErr;
      const [summaries, memories, semantic, procedural, relations, lastRun] = await Promise.all([
      ...
      const recurringPatterns = memories.filter(
        (m) =>
          m.isLatest &&
          m.type === "pattern" &&
