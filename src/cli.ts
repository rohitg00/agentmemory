...
function prepareRuntimeIiiConfig(configPath: string): string {
  ...
  const rendered = renderRuntimeDataDirIiiConfig(
    renderRuntimePortIiiConfig(raw),
...
async function passiveServerChecks(): Promise<DoctorCheck[]> {
  ...
  checks.push(
    ...
    },
    {
      ...
      hint: hasEmbed
        ? undefined
        : "Running BM25-only. Set EMBEDDING_PROVIDER=local, or EMBEDDING_PROVIDER=<remote> plus the matching provider key.",
    },
  );
