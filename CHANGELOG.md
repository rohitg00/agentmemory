...
- **The viewer dashboard no longer reloads once per live event.** Stream events and polling now schedule a single coalesced reload (at most one every 2 s) and a reload never runs twice at once; 40 observations in a burst used to fire 40 full dashboard fan-outs (about 400 requests) and could exhaust the browser's connection pool (#921, #1340). Live buffers on the Timeline and Activity tabs are capped at a few hundred entries so a large stream backlog cannot freeze the tab (#609).
