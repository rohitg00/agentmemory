...
- **Native iii v0.11.2 runtime config generation no longer writes unsupported top-level `port:`.** The bundled native engine accepts `workers`/`modules` config roots and rejects generated configs that start with `port: 49134`. `--port` and `--instance` now relocate agentmemory REST, streams, and viewer ports only; the native engine WebSocket remains on `49134` unless clients use an explicit `III_ENGINE_URL` or `III_ENGINE_PORT` for an externally managed engine.
...
- **Multi-instance port collisions on one host** ([#750](https://github.com/rohitg00/agentmemory/issues/750), [PR #815](https://github.com/rohitg00/agentmemory/pull/815)). `--port N` originally relocated only REST + viewer while streams (3112) stayed hardcoded. `loadConfig()` now derives streams = REST+1, so `--port 3211` selects REST 3211 and streams 3212. Native iii-engine v0.11.2 listen-port relocation is not supported by the bundled runtime; the engine WebSocket remains on 49134 unless clients use an explicit `III_ENGINE_URL` or `III_ENGINE_PORT` for an externally managed engine. New `--instance N` shortcut picks a 100-port REST block: `--instance 1` → REST 3211, streams 3212, viewer 3213. Per-port env overrides (`III_STREAM_PORT`, `III_ENGINE_PORT`, `III_ENGINE_URL`) still win for client configuration.
...
- **`--instance N` CLI flag** — REST/streams/viewer convenience. Picks a 100-port REST block off the 3111 base. Max N=50.
