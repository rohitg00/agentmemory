...
The MCP server exposes 55 tools by default (`--tools all`). Use `--tools core` (or `AGENTMEMORY_TOOLS=core`) for a lean 9-tool set on hosts with tight tool limits. The 9 core tools cover save, recall, smart search, sessions, export, audit, and governance delete.
