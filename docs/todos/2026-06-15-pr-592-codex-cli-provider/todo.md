...
| Merge prep | `$prep-merge-to-local-main` workflow | complete | Preflight found target branch active; no active hooks/signing config; local `main` clean at `d4393d1`; merged local `main` successfully. Post-merge `pnpm install --frozen-lockfile`/`pnpm test` are blocked by strict dependency-build approvals; direct local binary checks passed. |
...
- 2026-06-16: Committed the generated-reference fix as `04a0c4d` after `gitleaks protect --staged --redact` reported no leaks. Merged captured local `main` commit `d4393d1` successfully, creating merge commit `bfd7a7a`.
- 2026-06-16: Post-merge dependency setup with `CI=true pnpm install --frozen-lockfile` materialized packages but exited nonzero because `strictDepBuilds` blocked unapproved build scripts for `esbuild`, `onnxruntime-node`, `protobufjs`, and `sharp`. The command also wrote an unapproved `allowBuilds` template into `pnpm-workspace.yaml`; that setup artifact was removed without changing dependency policy.
- 2026-06-16: Because `pnpm` now re-runs the same dependency status check before `pnpm test`/`pnpm exec`, direct local binary checks were used for post-merge verification: `node_modules/.bin/tsx scripts/skills/generate.ts --check`, `node_modules/.bin/tsx scripts/skills/check.ts`, `node_modules/.bin/vitest run --exclude test/integration.test.ts` (159 files / 1993 tests), `node_modules/.bin/eslint ...`, and `git diff --check` all passed.
