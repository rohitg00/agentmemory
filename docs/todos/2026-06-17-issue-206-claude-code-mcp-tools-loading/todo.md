...
| Read-only issue validation | Repo-local evidence and public issue data | No | One of `close`, `already fixed`, `fix needed`, or `needs approval/defer` with file/test evidence | `fix needed`: public issue evidence showed Claude Code requested `2025-03-26` while the shim replied `2024-11-05`; plugin MCP exposure routes through the standalone shim; `${AGENTMEMORY_URL}` was already fixed but not sufficient | No real Claude Code plugin UI/session was run; client-side discard behavior remains inferred from public issue logs |
...
- Main-thread validation follow-up: a read-only Explorer subagent (`019ed5c4-5000-7410-a394-5da58a38db56`) validated the issue as `fix needed`. Evidence cited: pre-fix `src/mcp/standalone.ts` hardcoded `protocolVersion: SERVER_INFO.protocolVersion`; current `test/mcp-standalone.test.ts` covers Claude Code's `2025-03-26`, default fallback cases, and `tools/list` after negotiation; `plugin/.mcp.json` and `packages/mcp/bin.mjs` route plugin MCP startup through the standalone entrypoint; public issue data documents the protocol mismatch. The subagent's uncertainty is that no real Claude Code plugin UI/session was run.
