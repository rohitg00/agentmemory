...
| **Qoder CLI CN** (Qwen-derived) | `~/.qoder-cn/settings.json` | `agentmemory connect qoder-clicn` writes the standard `mcpServers` block (Windows allowed). Hook payload is field-compatible with Claude Code; wire the 10 supported lifecycle events via `hooks` in the same file — note `SubagentStart` and `TaskCompleted` are not fired by Qoder. See [QODER-COMPAT.md](QODER-COMPAT.md) for the full hook registry snippet and Windows path notes. |
