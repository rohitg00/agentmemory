...
| **DeepSeek Harness** | `$DSH_HOME/cordis.patch.yml` | `agentmemory connect dsh` appends an `@deepseek-ai/dsh-mcp-client` row to the home-level patch layer every Harness profile loads. Tools register as `mcp__agentmemory__*`. Defaults to `~/.dsh` when `DSH_HOME` is unset. |
