...
| | CLAUDE.md | claude-mem | agentmemory |
|---|---|---|---|
| Storage | Flat file | SQLite | iii-engine KV (persistent, distributed) |
| Capture | Manual | Partial hooks | All 12 hook types |
| Search | Text find | Full-text | Hybrid BM25 + vector (6 embedding providers) |
| Intelligence | None | Basic summaries | LLM compression, quality scoring, self-correction |
| Memory model | Append-only | Flat rows | Versioned with relationships and evolution |
| Forgetting | Manual delete | None | Auto-forget (TTL, contradictions, importance) |
| Multi-agent | One file | Per-agent DB | Shared KV with project-scoped profiles |
| Observability | None | None | Health monitor, circuit breaker, OTEL telemetry |
| Integration | Built-in | CLI tool | Plugin + MCP server + REST API + slash commands |
