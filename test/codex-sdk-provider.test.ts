...
describe("CodexSDKProvider", () => {
  ...
  it("invokes codex exec through stdin with the hook recursion guard set", async () => {
...
process.stdin.on("end", () => {
  ...
  if (!argv.includes("exec") || !argv.includes("--ephemeral") || !argv.includes("--ignore-rules") || !argv.includes("--skip-git-repo-check") || !argv.includes("-")) {
    console.error("missing expected codex exec flags: " + argv.join(" "));
    process.exit(8);
  }
...
`);
  ...
  it("surfaces codex exec failures", async () => {
    const command = makeFakeCodex(`
