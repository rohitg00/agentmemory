...
describe("viewer dashboard reliability", () => {
  it("stream events schedule one coalesced dashboard reload instead of a reload per event", () => {
    const router = extractFunction("routeWsMessage");
    expect(router).toMatch(/scheduleDashboardReload\(\)/);
    expect(router).not.toMatch(/loadDashboard\(\)/);
    expect(viewer).toMatch(/var DASHBOARD_RELOAD_COALESCE_MS = 2000;/);
  });
