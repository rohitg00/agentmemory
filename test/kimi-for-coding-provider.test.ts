...
describe("KimiForCodingProvider — base URL and header resolution", () => {
  const originalEnv = process.env["KIMI_FOR_CODING_BASE_URL"];

  afterEach(() => {
    if (originalEnv === undefined) {
      delete process.env["KIMI_FOR_CODING_BASE_URL"];
    } else {
      process.env["KIMI_FOR_CODING_BASE_URL"] = originalEnv;
    }
  });
  ...
  it("honors KIMI_FOR_CODING_BASE_URL via getEnvVar", () => {
