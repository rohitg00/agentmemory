...
describe("doctor v2 diagnostic catalog", () => {
  ...
  it("dryRunPlan reports manual-only diagnostics as manual fixes", () => {
    const diagnostics = buildDiagnostics(stubEffects());
    const manual = diagnostics.find((d) => d.id === "iii-on-path-not-local-bin")!;
    const lines = dryRunPlan(stubCtx(), [
      { diagnostic: manual, status: { ok: false, detail: "stub fail" } },
    ]);
    expect(lines.some((l) => l.includes("manual fix:"))).toBe(true);
    expect(lines.some((l) => l.includes("would fix:"))).toBe(false);
  });
