...
engine binary in from the official `iiidev/iii` image. Tagged releases
also publish a pre-built multi-arch image to
`ghcr.io/rohitg00/agentmemory`, with tags like `0.9.29`, `0.9`, and
`latest`. Storage mounts at `/data`; an HMAC secret is generated by the
first-boot entrypoint and persisted to the volume. The entrypoint
overwrites the npm-bundled iii config with a
