...
The managed-platform templates ship self-contained Dockerfiles that
pull `@agentmemory/agentmemory` from npm at build time and copy the iii
engine binary in from the official `iiidev/iii` image. The Helm chart
uses that same image shape but expects you to build and push the image
to a registry your cluster can pull from. Storage mounts at `/data`; an
HMAC secret is generated by the first-boot entrypoint and persisted to
the volume. The entrypoint overwrites the npm-bundled iii config with a
deploy-tuned one that binds `0.0.0.0` and uses absolute `/data` paths,
then drops privileges from `root` to `node` via `gosu` before exec'ing
the agentmemory CLI.
...
| [Kubernetes / Helm](./helm/agentmemory/README.md) | Bring your own cluster and registry image. Mirrors the same single-container `/data` pattern with a PVC and REST-only Service. | Cluster + registry cost |
...
- Pick **Kubernetes / Helm** if you already operate a cluster and want
  ...
  NetworkPolicy. Build and push an image from one of the deploy
  Dockerfiles first; this repository does not publish an official
  Kubernetes image as part of the chart.

All deployments give you the same agentmemory API at the same port
(3111) with the same auth model. Migrating between them later is a
`tar` of `/data` and a re-import — see each platform's README for the
exact commands.
