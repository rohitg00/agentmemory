...
function writeHookPayloadTemp(input) {
	mkdirSync(HOOK_PAYLOAD_DIR, { recursive: true });
	try {
		chmodSync(HOOK_PAYLOAD_DIR, 448);
	} catch {}
...
function readWorkerHookPayload() {
...
//#endregion
//#region src/hooks/cursor/run-detached.ts
const IS_WORKER = process.env["AM_HOOK_WORKER"] === "1";
const hookKey = process.argv[2];
function runWorker() {
	const watchdog = setTimeout(() => process.exit(0), hookKey === "sessionEnd" ? 25e4 : 13e4);
	try {
...
async function runParent() {
	...
	const payloadFile = writeHookPayloadTemp(await readHookStdinComplete());
	...
	child.unref();
	const bail = setTimeout(() => process.exit(0), 2e3);
...
//# sourceMappingURL=run-detached.mjs.map