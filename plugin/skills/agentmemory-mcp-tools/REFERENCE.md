...
| `memory_lineage` |  | `query`*: string, `limit`: number, `since`: string, `until`: string, `channels`: string, `includeAdjacentTurns`: boolean, `includeGraph`: boolean, `order`: string | Concept lineage: chronologically-ordered hits for a phrase across observation, memory, lesson, and summary channels. Use to trace when a term first entered the corpus and what surrounded it. |
