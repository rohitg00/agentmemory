...
| `memory_lesson_supersede` |  | `lessonId`*: string, `replacementLessonId`*: string, `reason`*: string, `project`: string, `expectedUpdatedAt`: string, `actor`: string | Mark one lesson superseded in favor of an active replacement in the same durable scope, preserving legacy tombstone fields, lineage, and an audit trail. |
