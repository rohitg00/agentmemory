...
| `memory_lesson_delete` |  | `lessonId`*: string, `reason`*: string, `project`: string, `expectedUpdatedAt`: string, `actor`: string | Retract one lesson by exact ID when its evidence is invalid, preserving the legacy tombstone fields, correction metadata, and audit trail. Requires a human-readable reason. |
| `memory_lesson_recall` | yes | `query`*: string, `project`: string, `minConfidence`: number, `limit`: integer, `retrievalMode`: string, `compact`: boolean, `mechanismId`: string, `claimType`: string, `evidenceVerdicts`: array, `structuredFacets`: object, `tags`: array, `scopeRing`: string, `sensitivity`: string | Search causal lessons. Legacy-compatible lexical ranking is the default; retrievalMode=hybrid adds semantic similarity after authorization and exact structured prefilters, with deterministic lexical fallback. Use compact=true to inject bounded causal lessons instead of full evidence rows. |
| `memory_lesson_save` | yes | `content`*: string, `context`: string, `confidence`: number, `project`: string, `tags`: string, `mechanismId`: string, `mechanismVersion`: string, `mechanismAliases`: array, `claim`: string, `claimType`: string, `evidenceVerdict`: string, `lifecycle`: string, `applicabilityConditions`: array, `nonApplicabilityConditions`: array, `falsificationConditions`: array, `structuredFacets`: object, `evidenceRefs`: array, `scope`: object, `sensitivity`: string, `reviewAfter`: string, `contradictedByLessonIds`: array | Save a legacy prose lesson or a structured causal lesson. Reinforcement tracks reuse without changing evidence confidence; structured inputs support verdict, lifecycle, durable evidence, scope, sensitivity, and deterministic fingerprints. |
| `memory_lesson_supersede` |  | `lessonId`*: string, `replacementLessonId`*: string, `reason`*: string, `project`: string, `expectedUpdatedAt`: string, `actor`: string | Mark one lesson superseded in favor of an active replacement in the same durable scope, preserving legacy tombstone fields, lineage, and an audit trail. An optional project guard applies only to the original lesson; replacement compatibility follows durable scope rules. |
