...
Compatibilidade: este release tem como alvo o `iii-sdk` 0.22.1 e fixa o iii-engine em v0.22.1.
...
Aviso: este comando muta o workspace/runtime atual. Pode atualizar dependências JavaScript e puxar a imagem Docker fixada `iiidev/iii:0.22.1`. Nunca instala um engine iii sem fixação ou mais recente.
...
- **Windows:** baixe `iii-x86_64-pc-windows-msvc.zip` de [iii-hq/iii releases v0.22.1](https://github.com/iii-hq/iii/releases/tag/iii%2Fv0.22.1), extraia `iii.exe`, adicione ao PATH

Ou use Docker (o `docker-compose.yml` empacotado puxa `iiidev/iii:0.22.1`). Docs completas: [iii.dev/docs](https://iii.dev/docs).
...
> Nota: o **engine** iii é um binário pré-compilado, não um crate do cargo, então não tente instalá-lo com `cargo install`. (Os **SDKs** do iii são publicados no crates.io, npm e PyPI, mas o agentmemory não precisa deles.) Métodos de instalação do engine suportados, todos fixados em v0.22.1: o binário pré-compilado v0.22.1 acima, o script de instalação `sh` upstream **com a fixação de versão** `curl -fsSL https://install.iii.dev/iii/main/install.sh | VERSION=0.22.1 sh` (macOS/Linux) e a imagem Docker `iiidev/iii:0.22.1`. Um simples `install.sh | sh` instala o engine **mais recente**, que o agentmemory não suporta; sempre passe `VERSION=0.22.1`. O mais fácil de tudo: basta rodar `npx @agentmemory/agentmemory`, que busca o engine fixado em `~/.agentmemory/bin` para você.
...
`iii-config.yaml` vem com o worker `iii-observability` habilitado (`exporter: memory`, `sampling_ratio: 0.1`, métricas + logs). Sem configuração extra; no momento em que o agentmemory inicia, toda operação de memória emite um log estruturado que o console consegue ler, e uma em cada dez (`sampling_ratio: 0.1`) também emite um trace span.
...
| [`database`](https://workers.iii.dev/workers/database) | Adaptador de estado baseado em SQL quando você ultrapassa o KV in-memory padrão |
