...
<p align="center">
  <img src="https://img.shields.io/badge/MCP-55_tools-1f6feb?style=flat-square" alt="55 MCP tools" />
...
an MCP server with all 55 memory tools:
...
This gives Hermes access to all 55 MCP tools and enables the agentmemory memory provider. Start the server separately:
...
| `AGENTMEMORY_REQUIRE_HTTPS` | (off) | When set to `1`, refuse to send the bearer token over plaintext HTTP to a non-loopback host. Sends only when `AGENTMEMORY_URL` is `https://...` or points at `localhost`/`127.0.0.1`/`::1`. With this off, the plugin warns once on stderr and skips the unsafe request. |
