...
| `AGENTMEMORY_REQUIRE_HTTPS` | (off) | When set to `1`, refuse to send either credential over plaintext HTTP to a non-loopback host. Sends only when `AGENTMEMORY_URL` is `https://...` or points at `localhost`/`127.0.0.1`/`::1`. With this off, the plugin warns once on stderr but still sends. |
